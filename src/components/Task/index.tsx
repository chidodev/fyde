...
const Task: React.FC<{ task: ITask }> = ({ task }) => {
    ...
    return (
        <div className='w-full z-10 my-1 shadow-lg drop-shadow-xl rounded px-3 py-2 flex items-center justify-between'>
            <div className='flex items-center'>
                <div onClick={() => setTaskSelected(!taskSelected)} className={`border rounded-full w-6 h-6 flex items-center`}>
                ...
                <span className='font-semibold text-slate-800 ml-3'>{task.name}</span>
            </div>
                ...
                {showContextMenu &&
                    (
                        <div className='flex flex-col absolute right-1 top-8'>
                            ...
                            <div className='flex z-50 flex-col  rounded-lg bg-blue-950 text-white py-4'>
                    ...
                    <AvatarGroup max={4} className=' mx-3'>
                        {
                            task.assignees.map((assignee: number, index: number) => {
                                ...
                                return (
                                    <Avatar style={{ zIndex: 0 }} key={index} alt={user.names} src={user.profile} />
                                )
                            })
                        }
                    </AvatarGroup>
